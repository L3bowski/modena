...
const registerApp = (server, modenaConfig, appConfig) => {
	...
	try
	{
		...
		if (appConfig.indexFilePath != null) {
			...
			routerPromise = tracedConfigureRoute(appMiddleware, appUtils);

			if (!(routerPromise instanceof Promise)) {
				routerPromise = Promise.resolve(routerPromise);
			}
		}

		return routerPromise.then(appRouter => {
